...
public:
    ...
    struct StrategyStats {
        ...
        // 详细的qv和qtheta编码统计
        int qv_total_bits = 0;
        int qtheta_total_bits = 0;
        std::vector<int64_t> qv_values;
        std::vector<int64_t> qtheta_values;
        std::vector<int> qv_bit_lengths;
        std::vector<int> qtheta_bit_lengths;
        ...
        void PrintDetailedEncodingStats() const;
    };
...
private:
    ...
    // 新增的几何直接求解辅助函数
    double AngularDiff(double angle1, double angle2) const;
    double CalculateEliasGammaCost(int64_t value) const;
    GpsPoint CalculateProjectionOnLine(const GpsPoint& point, const GpsPoint& line_start, const GpsPoint& line_end) const;
};
