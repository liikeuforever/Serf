...
int main() {
    std::cout << "=== GPS轨迹压缩详细策略分析 (10万个点) ===" << std::endl;
    ...
    SerfQtGpsStatsCompressor compressor(points.size(), error_bound, epsilon_v, epsilon_theta);
    ...
    for (size_t i = 0; i < std::min(points.size(), size_t(1000)); ++i) { // 只验证前1000个点以节省时间
        auto decompressed_point = decompressor.GetNextGpsPoint();
        
        double dx = points[i].longitude - decompressed_point.longitude;
        double dy = points[i].latitude - decompressed_point.latitude;
    ...
    std::cout << "验证点数: 1000" << std::endl;
    std::cout << "最大误差: " << std::scientific << max_error << " 度" << std::endl;
    std::cout << "误差违反: " << error_count << "/1000" << std::endl;
    
    if (error_count == 0) {
        std::cout << "✅ 解压验证通过" << std::endl;
    } else {
        std::cout << "⚠️  发现 " << error_count << " 个误差违反" << std::endl;
    }
