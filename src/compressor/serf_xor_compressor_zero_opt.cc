#include "serf_xor_compressor_zero_opt.h"
#include "utils/elias_gamma_codec.h"
#include "utils/post_office_solver.h"

SerfXORCompressorZeroOpt::SerfXORCompressorZeroOpt(int windows_size, double max_diff, long adjust_digit) :
...
void SerfXORCompressorZeroOpt::AddValue(double v) {
  uint64_t this_val;
...
long SerfXORCompressorZeroOpt::compressed_size_last_block() const {
  return compressed_size_last_block_;
}

Array<uint8_t> SerfXORCompressorZeroOpt::compressed_bytes_last_block() {
  return compressed_bytes_last_block_;
}

Array<uint8_t>& SerfXORCompressorZeroOpt::compressed_bytes() {
  return compressed_bytes_last_block_;
}

void SerfXORCompressorZeroOpt::Close() {
  // Flush any remaining zero sequence before writing the final NaN marker
...
int SerfXORCompressorZeroOpt::CompressValue(uint64_t value) {
  int this_size = 0;
  uint64_t xor_result = stored_val_ ^ value;

  if (SERF_UNLIKELY(xor_result == 0)) {
    // Zero XOR result - start or continue zero sequence
    if (!in_zero_sequence_) {
    ...
    // Non-zero XOR result - end zero sequence if active
    if (in_zero_sequence_) {
...
int SerfXORCompressorZeroOpt::UpdatePositionsIfNeeded() {
  int len;
...
int SerfXORCompressorZeroOpt::FlushZeroSequence() {
  if (!in_zero_sequence_ || zero_sequence_count_ == 0) {
    return 0;
  }
  
  // Encode the count of zeros using Elias Gamma encoding
