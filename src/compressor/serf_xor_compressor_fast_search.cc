#include "serf_xor_compressor_fast_search.h"
#include "utils/serf_utils_64_fast_simple.h"

SerfXORCompressorFastSearch::SerfXORCompressorFastSearch(int windows_size, double max_diff, long adjust_digit) :
...
void SerfXORCompressorFastSearch::AddValue(double v) {
  uint64_t this_val;
  ...
  if (SERF_LIKELY(std::abs(Double::LongBitsToDouble(stored_val_) - kAdjustDigit - v) > kMaxDiff)) {
    // Use fast approximator search instead of exhaustive search
...
long SerfXORCompressorFastSearch::compressed_size_last_block() const {
  return compressed_size_last_block_;
}

Array<uint8_t> SerfXORCompressorFastSearch::compressed_bytes_last_block() {
  return compressed_bytes_last_block_;
}

Array<uint8_t>& SerfXORCompressorFastSearch::compressed_bytes() {
  return compressed_bytes_last_block_;
}

void SerfXORCompressorFastSearch::Close() {
...
int SerfXORCompressorFastSearch::CompressValue(uint64_t value) {
  int this_size = 0;
...
int SerfXORCompressorFastSearch::UpdatePositionsIfNeeded() {
  int len;
